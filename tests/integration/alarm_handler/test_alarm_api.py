...
#     with open(f"/app/tests/resources/tmc_alarm_rules/{filename}", "rb") as file:
#         response = httpx.post(
...
#         assert response_data["alarm summary"]["tag"] == ["CentralNode_telescopestate_standby", "SubarrayNode_obsstate_fault"]
...
#     response = httpx.post(
#         f"http://alarm-handler-configurator.{namespace}.svc.cluster."
#         + "local:8004/remove-alarm?tag=test1&alarmhandlerfqdn=alarm%2Fhandler%2F01",
#         data={"tag": "SubarrayNode_obsstate_fault", "alarmhandlerfqdn": "alarm/handler/01"},
#     )
#     response_data = response.json()
#     assert response_data["alarm summary"]["tag"] != ["SubarrayNode_obsstate_fault"]
