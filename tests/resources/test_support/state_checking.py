...
class StateChecker:
    ...
    def _loop(
        self, resources, state_name, resolution, max_nr_of_records, specific_states
    ):
        n = 0
        list = []
        while self._get_running():
            ...
            for resource_name in resources:
                if specific_states == {}:
                    ...
                    specific_state_name = self._get_specific_state(
                        specific_states, resource_name, state_name
                    )
                record["{} state".format(resource_name)] = resource(resource_name).get(
                    specific_state_name
                )
                record["{} delta".format(resource_name)] = "{:f}".format(
                    time() - start_time
                )
            ...
            if max_nr_of_records != None:
                if n >= max_nr_of_records:
                    logging.debug(
                        "nr of records ({}) is above max {}, stopping".format(
                            n, max_nr_of_records
                        )
                    )
                    self._clear_running()
