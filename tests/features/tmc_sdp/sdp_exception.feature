Feature: TMC SubarrayNode handles the exception when AssignResources command fails on SDP Subarray
    @XTP-29381 @tmc_sdp
    Scenario Outline: TMC SubarrayNode handles the exception duplicate eb-id raised by SDP subarray
        Given a TMC
        ...
        And TMC executes second AssignResources command with duplicate eb-id
