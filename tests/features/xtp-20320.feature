Feature: Default
	...
	@XTP-20320 @XTP-28347 @End_to_end @Resource_Management
	Scenario: Successfully execute a scan after a failed attempt to assign resources
		...
		When I issue the command AssignResources passign a correct JSON script
		Then the subarray transitions to obsState IDLE
		When I issue the command Configure passign a correct JSON script  
